...
import { editRoom, fetchRooms } from "../../redux/features/RoomsSlice";
...
const EditRoomModal = ({ open, onClose, room }) => {
  ...
  useEffect(() => {
    const { details } = room;
    const { images, name, location, site, ...others } = details;
    setImageURLs(images);
    setName(name);
    ...
    Object.keys(others).map((key) => {
      newInputs.push({ key: key, value: others[key] });
    });
    setInputs(newInputs);
  }, [room]);
  useEffect(() => {
  ...
  const handleEditRoom = async () => {
    ...
    dispatch(editRoom({ room_id: room.room_id, data })).then((result) => {
      if (result.payload) {
        setLoading(false);
        onClose();
        setImages([]);
      ...
      <Modal open={open} onClose={onClose}>
        <div className="flex flex-col gap-7 w-[700px] h-[400px] ">
          ...
          <div className="w-full flex justify-end gap-6">
            ...
            <button className="btn btn-danger" onClick={handleEditRoom}>
              Submit
            </button>
          </div>
        </div>
